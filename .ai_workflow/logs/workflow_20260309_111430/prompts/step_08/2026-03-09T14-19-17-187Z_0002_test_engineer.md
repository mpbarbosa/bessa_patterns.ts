...
**Timestamp:** 2026-03-09T14:19:17.187Z
...
```
**E2E Test Strategy & Implementation for bessa_patterns.ts**
...
- Multi-step workflows (registration, checkout, onboarding)
...
test('Dashboard visual regression', async ({ page }) => {
  ...
  expect(screenshot).toMatchImageSnapshot({ customDiffConfig: { threshold: 0.01 } });
});
```
...
test('Mobile login', async ({ page }) => {
  await page.emulate({ device: 'iPhone 12' });
  await page.goto('/login');
  // ...login steps
});
```
...
**Measure LCP, FID, CLS:**
- Use Lighthouse CI or Playwright trace viewer
...
jobs:
  lighthouse:
    ...
    steps:
      ...
      - run: lhci autorun --collect.url=https://your-app-url
```
...
**Parallel Execution:**
- Use Playwright's built-in parallelism
- Split tests by project or file

---

### 7. Flaky Test Prevention & Debugging

- Use `waitForSelector`, not arbitrary timeouts
- Isolate test data (unique per test)
- Clean up after each test (`afterEach`)
- Capture screenshots/videos on failure

**Example:**
```typescript
test('Wait for dashboard', async ({ page }) => {
  await page.goto('/dashboard');
...
- Create reusable helpers/utilities
...
- [x] Maintainable test structure (POM, helpers)

**Action Items:**
1. Implement Page Objects and helper utilities.
2. Add visual regression and accessibility tests.
3. Configure cross-browser and device emulation.
4. Integrate E2E tests into CI/CD pipeline.
5. Monitor and optimize test reliability and performance.

**Estimated Effort:**  
- Initial setup: 2-4h  
- Critical user journeys: 4-8h  
- Visual/accessibility/performance: 2-4h  
- CI/CD integration: 1-2h  
- Ongoing maintenance: as needed

**Best Practices:**  
- Use stable selectors, explicit waits, and test isolation.
- Run E2E tests on every commit/PR.
- Review visual diffs and accessibility reports regularly.
