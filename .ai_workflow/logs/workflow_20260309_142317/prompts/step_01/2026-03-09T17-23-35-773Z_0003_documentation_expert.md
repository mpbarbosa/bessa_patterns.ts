...
**Timestamp:** 2026-03-09T17:23:35.773Z
...
**Your assigned file (EDIT THIS FILE ONLY)**: CHANGELOG.md

> You are responsible for ONE file: `CHANGELOG.md`.
> All other files in "Changed files" are read-only context to inform your edits.
> Do NOT provide before/after edit blocks for any file other than `CHANGELOG.md`.
...
2. **Identify documentation impact**: Determine what in `CHANGELOG.md` needs updating based on the changed files
3. **Determine changes**: Identify exact sections of `CHANGELOG.md` requiring updates
4. **Verify accuracy**: Ensure examples and references in `CHANGELOG.md` are still correct

**OUTPUT FORMAT**:
- Provide before/after edit blocks ONLY for `CHANGELOG.md`
- If no changes needed to `CHANGELOG.md`, explicitly say "No updates required"
...
2. **Scope**: Edit ONLY `CHANGELOG.md`. Use all other file contents as read-only context.
   Do not produce before/after blocks for any file other than `CHANGELOG.md`.
...
## Scope Notes
- All changelog entries, version numbers, and references in `CHANGELOG.md` are accurate and consistent with the current project state. No changes needed.
```